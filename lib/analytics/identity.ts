 ...
 *
 * Uses an RFC 6234 standard pure JavaScript SHA-256 implementation so identity
 * derivation is 100% portable, requires zero native modules, and never crashes
 * development clients, test builds, or Expo Go with missing native dependencies.
 */
...
/**
 * RFC 6234 standard SHA-256 implementation in pure JavaScript.
 * Produces identical hex output to crypto.subtle and expo-crypto without native dependencies.
 */
export function pureJsSha256(ascii: string): string {
  ...
  for (let i = 0; i < ascii.length; i++) {
    ...
    if (code < 0x80) {
      ...
      i++;
      const code2 = 0x10000 + (((code & 0x3ff) << 10) | (ascii.charCodeAt(i) & 0x3ff));
...
export async function deriveDistinctId(userUuid: string): Promise<string> {
  ...
  return pureJsSha256(buildDistinctIdInput(userUuid));
}
...
export async function deriveGroupKey(groupUuid: string): Promise<string> {
  ...
  return pureJsSha256(buildGroupKeyInput(groupUuid));
}
