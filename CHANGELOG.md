...
- Distilled unused theme exports, dead realtime hooks, and currency conversion functions:
  - Cleaned up raw color exports in `constants/theme.ts`: deleted unused exports (`ACCENT_PURPLE`, `ACCENT_YELLOW`, `SUCCESS_DARK`, `BG_CARD_DARK`, `BG_GLASS_DARK`, `BG_ICON_SUCCESS_DARK`, `BTN_CLOSE_DARK`, `BTN_CLOSE_LIGHT`) and encapsulated internal palette constants (`ACCENT_GREEN`, `SUCCESS_LIGHT`, `ERROR_COLOR`, `BG_DARK`, `BG_LIGHT`, `BG_CARD_LIGHT`, `BORDER_DARK`, `BORDER_LIGHT`, `TEXT_DARK`, `TEXT_LIGHT`, `TEXT_SECONDARY_DARK`, `TEXT_SECONDARY_LIGHT`), preserving only theme-token-consumed exports (`ACCENT_TEAL`, `BG_ICON_DARK`, `BG_ICON_LIGHT`, `BORDER_ACCENT_DARK`, `BORDER_ACCENT_LIGHT`, `BTN_DISABLED_DARK`, `BTN_DISABLED_LIGHT`).
  - Removed dead realtime wrapper hooks (`useGroupExpensesRealtime` and `useFriendExpensesRealtime`) from `hooks/use-realtime.ts`, retaining the active `useRealtime` and `useGroupsHomeRealtime` hooks.
  - Removed unused speculative currency conversion logic (`convertCurrency` and `EXCHANGE_RATES`) from `utils/currency.ts` and pruned obsolete test assertions from `utils/currency.test.ts`.
