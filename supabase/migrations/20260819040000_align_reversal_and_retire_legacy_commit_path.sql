-- Keep reversal on the operation-relative sign contract and retire the
-- historical internal/combined command implementations after 19020000.
...
BEGIN
  ...
  expected_after_balance := CASE WHEN operation_row.expected_balance = 0 THEN 0
    ELSE operation_row.expected_balance
      - SIGN(operation_row.expected_balance) * operation_row.requested_payment_amount END;
...
DROP FUNCTION IF EXISTS public.commit_settlement_operation_internal(
