-- Groups home read model.
--
-- The function returns only group-card fields and the authenticated user's
-- balance. It intentionally performs the balance aggregation as the function
-- owner so the home screen does not download full group histories or trigger
-- the expensive per-row RLS helper chain.
...
BEGIN
  ...
  settlement_impacts AS (
    SELECT
      settlement.group_id,
      CASE
        WHEN settlement.from_user_id = app_user_id THEN settlement.amount
    ...
    WHERE settlement.from_user_id = app_user_id
       OR settlement.to_user_id = app_user_id
  ),
  all_impacts AS (
    SELECT group_id, impact_amount
  ...
  balances AS (
    SELECT
      group_id,
      SUM(impact_amount) AS computed_balance
    FROM all_impacts
    GROUP BY group_id
  )
  SELECT
    ...
    CASE
      WHEN ABS(COALESCE(balance.computed_balance, 0)) < 0.01 THEN 0
      ELSE COALESCE(balance.computed_balance, 0)
    END AS your_balance
