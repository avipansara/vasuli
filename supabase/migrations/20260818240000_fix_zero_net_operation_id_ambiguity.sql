-- Qualify the zero-net operation variable in transfer-row queries.
...
BEGIN
  ...
  RETURNING id INTO zero_net_operation_block.operation_id;

  IF zero_net_operation_block.operation_id IS NULL THEN
    ...
    END IF;
    zero_net_operation_block.operation_id := existing_operation.id;
  END IF;
  ...
  LOOP
    ...
    IF transfer_group_id IS NULL
       ...
       OR transfer_delta IS NULL OR transfer_delta = 0
    ...
    INSERT INTO public.settlement_scope_transfers (
      ...
      zero_net_operation_block.operation_id, transfer_group_id,
      transfer_from_user_id, transfer_to_user_id, transfer_currency,
      transfer_delta, NULLIF(transfer->>'note', '')
  ...
  WHERE t.operation_id = zero_net_operation_block.operation_id;
  ...
  RETURN jsonb_build_object(
    ...
    'settlements', '[]'::jsonb, 'operationId', zero_net_operation_block.operation_id,
