-- Extend the operation RPC with explicit non-cash scope transfers. The
-- existing nine-argument function remains available for compatibility; this
-- overload wraps it in the same transaction and appends validated transfers.
...
BEGIN
  ...
  LOOP
    ...
    ON CONFLICT (operation_id, group_id) DO NOTHING;
  END LOOP;
