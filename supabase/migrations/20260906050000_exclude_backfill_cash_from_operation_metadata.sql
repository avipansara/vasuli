-- Backfill-created settlement rows mirror historical scope transfers. They
-- remain readable for compatibility but must not establish operation cash or
-- local payment attribution in activity metadata.
...
-- The Friend detail RPC also carries settlement rows used by the activity
-- projection. Add the marker to both its direct and group settlement JSON
-- without copying and forking the large read-model function definition.
DO $$
...
BEGIN
  ...
  IF function_definition IS NULL THEN
    RAISE EXCEPTION 'Friend detail read model function is required before marker rollout';
  END IF;

  function_definition := replace(
    function_definition,
    '''operationId'', s.operation_id,',
    '''operationId'', s.operation_id, ''backfilledTransferId'', s.backfilled_transfer_id,'
  );
