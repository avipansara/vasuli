-- Development hotfix for 20260819020000.
-- The applied function attempted to INSERT into the PL/pgSQL variable name
-- new_request_fingerprint instead of the settlement_operations column
-- request_fingerprint, producing SQLSTATE 42703 before idempotency could run.
-- Keep this replacement explicit and additive; do not edit the prior migration.
...
BEGIN
  ...
  IF new_operation_id IS NULL THEN
    ...
    IF existing_operation.request_fingerprint IS NULL THEN
      ...
      ) ORDER BY s.created_at, s.id), '[]'::jsonb) INTO stored_allocations
      ...
      ) ORDER BY ordinality), '[]'::jsonb) INTO requested_allocations
      ...
      SELECT COALESCE(jsonb_agg(jsonb_build_object(
        ...
        'signedGroupBalanceDelta', to_jsonb(t.signed_group_balance_delta), 'note', to_jsonb(t.note)
      ) ORDER BY t.created_at, t.id), '[]'::jsonb) INTO stored_transfers
      ...
      ) ORDER BY ordinality), '[]'::jsonb) INTO requested_transfers
      ...
      IF stored_allocations <> requested_allocations OR stored_transfers <> requested_transfers THEN
        RAISE EXCEPTION 'SETTLEMENT_PAYMENT_INTENT_REUSED_WITH_DIFFERENT_PAYMENT';
      END IF;
      UPDATE public.settlement_operations SET request_fingerprint = new_request_fingerprint
  ...
  ELSE
    ...
    PERFORM 1 FROM public.users u WHERE u.id IN (app_user_id, p_friend_id) ORDER BY u.id FOR UPDATE;
    ...
    IF ROUND(current_balance, 2) <> ROUND(p_expected_balance, 2) THEN RAISE EXCEPTION 'SETTLEMENT_STALE_BALANCE'; END IF;
    ...
    FOR transfer IN SELECT value FROM jsonb_array_elements(COALESCE(p_transfers, '[]'::jsonb)) LOOP
      ...
      END IF;
      IF NOT EXISTS (SELECT 1 FROM public.group_members actor_member
        JOIN public.group_members friend_member ON friend_member.group_id = actor_member.group_id
          AND friend_member.user_id = p_friend_id
      ...
      transfer_deltas_by_group := jsonb_set(transfer_deltas_by_group,
        ARRAY[transfer_group_id::TEXT], to_jsonb(COALESCE((transfer_deltas_by_group->>transfer_group_id::TEXT)::NUMERIC, 0) + transfer_delta));
    ...
    FOR allocation IN SELECT value FROM jsonb_array_elements(p_allocations) LOOP
      ...
      IF allocation_amount IS NULL OR allocation_amount <= 0 OR allocation_amount <> ROUND(allocation_amount, 2)
      ...
      END IF;
      IF allocation_group_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.group_members actor_member
        JOIN public.group_members friend_member ON friend_member.group_id = actor_member.group_id
          AND friend_member.user_id = p_friend_id
        WHERE actor_member.group_id = allocation_group_id AND actor_member.user_id = app_user_id)
      THEN RAISE EXCEPTION 'SETTLEMENT_GROUP_SCOPE_INVALID'; END IF;
      ...
      ELSIF allocation_group_id::TEXT = ANY(seen_group_ids) THEN
        RAISE EXCEPTION 'SETTLEMENT_ALLOCATION_INVALID';
      ELSE seen_group_ids := array_append(seen_group_ids, allocation_group_id::TEXT); END IF;

      IF allocation_group_id IS NULL THEN
        SELECT COALESCE(SUM(CASE WHEN e.paid_by = app_user_id THEN COALESCE(friend_split.amount, 0)
          WHEN e.paid_by = p_friend_id THEN -COALESCE(current_split.amount, 0) ELSE 0 END), 0)
          + COALESCE((SELECT SUM(CASE WHEN s.from_user_id = app_user_id THEN s.amount ELSE -s.amount END)
            FROM public.settlements s WHERE s.group_id IS NULL AND s.currency = p_currency
              AND ((s.from_user_id = app_user_id AND s.to_user_id = p_friend_id)
                OR (s.from_user_id = p_friend_id AND s.to_user_id = app_user_id))), 0)
        INTO current_scope_balance FROM public.expenses e
        LEFT JOIN public.expense_splits current_split ON current_split.expense_id = e.id
          AND current_split.user_id = app_user_id AND (current_split.amount > 0 OR e.paid_by = app_user_id)
        LEFT JOIN public.expense_splits friend_split ON friend_split.expense_id = e.id
          AND friend_split.user_id = p_friend_id AND (friend_split.amount > 0 OR e.paid_by = p_friend_id)
        WHERE e.deleted_at IS NULL AND e.group_id IS NULL AND e.currency = p_currency
          AND (COALESCE(current_split.amount, 0) > 0 OR e.paid_by = app_user_id)
      ...
      END IF;
      IF current_scope_balance = 0 OR (current_scope_balance > 0 AND allocation_from_user_id <> p_friend_id)
      ...
      IF settlement_direction IS NULL THEN settlement_direction := CASE WHEN allocation_from_user_id = app_user_id
    ...
    VALUES (p_payment_intent_id, app_user_id, p_friend_id, p_amount, p_currency, p_date) RETURNING id INTO commitment_id;
    FOR allocation IN SELECT value FROM jsonb_array_elements(p_allocations) LOOP
      ...
      VALUES (NULLIF(allocation->>'groupId', '')::UUID, (allocation->>'fromUserId')::UUID,
        (allocation->>'toUserId')::UUID, (allocation->>'amount')::NUMERIC, allocation->>'currency',
        p_date, commitment_id, new_operation_id);
    END LOOP;
    FOR transfer IN SELECT value FROM jsonb_array_elements(COALESCE(p_transfers, '[]'::jsonb)) LOOP
      ...
      VALUES (new_operation_id, NULLIF(transfer->>'groupId', '')::UUID,
        NULLIF(transfer->>'fromUserId', '')::UUID, NULLIF(transfer->>'toUserId', '')::UUID,
        transfer->>'currency', (transfer->>'signedGroupBalanceDelta')::NUMERIC,
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object('id', s.id, 'operationId', s.operation_id,
    ...
    INTO requested_allocations FROM public.settlements s WHERE s.operation_id = new_operation_id;
  SELECT s.created_at INTO commitment_created_at FROM public.settlement_commitments s WHERE s.id = commitment_id;
  ...
  FROM (SELECT DISTINCT s.group_id FROM public.settlements s
    WHERE s.operation_id = new_operation_id AND s.group_id IS NOT NULL
    UNION SELECT DISTINCT t.group_id FROM public.settlement_scope_transfers t
    WHERE t.operation_id = new_operation_id AND NOT t.is_reversal) groups;
  RETURN jsonb_build_object('paymentIntentId', p_payment_intent_id, 'reused', operation_reused,
    'committedAt', commitment_created_at, 'totalAmount', p_amount, 'currency', p_currency,
    'direction', settlement_direction, 'settlements', requested_allocations,
    'operationId', new_operation_id, 'mode', p_mode, 'affectedGroupIds', affected_group_ids,
