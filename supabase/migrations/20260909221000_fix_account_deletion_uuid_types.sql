-- Correct the account deletion function's local identifier types. The
-- underlying user and group identifiers are UUIDs, so comparing them as text
-- fails at runtime on PostgreSQL.
